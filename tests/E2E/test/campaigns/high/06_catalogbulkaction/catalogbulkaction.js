...
scenario('Catalog bulk action', () => {
  ...
  scenario('delete  the  duplicate products', client => {
    ...
    /* test ('Should choose the number of objects per page',()=>{
       return promise
           .then(() => client.waitForExistAndClick(ProductList.pagination_select))
           .then(() => client.waitAndSelectByValue(ProductList.pagination_select, '100'))});*/
  }, 'catalogbulkaction');
  ...
  /*scenario('Logout from the back Office', client => {
    test('should logout successfully from the Back Office', () => {
      return promise
          .then(() => client.scrollTo(AccessPageBO.sign_out))
