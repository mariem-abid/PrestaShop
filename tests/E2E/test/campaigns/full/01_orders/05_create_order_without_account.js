...
let dateSyst = dateFormat(new Date(), 'yyyy-mm-dd');
...
scenario('Create order by a guest from the Front Office', client => {
  ...
  scenario('Check the "' + productData[0].name + date_time + '" in the Front Office', client => {
    ...
    test('should check the existance of the product picture', () => client.isExisting(CheckoutOrderPage.modal_product_picture));
  ...
  scenario('Check the "' + productData[1].name + date_time + '" in the Front Office', client => {
    test('should go to the Home page', () => client.waitForExistAndClick(AccessPageFO.logo_home_page, 1000));
    test('should search for the product "' + productData[1].name + date_time + '"', () => client.searchByValue(SearchProductPage.search_input, SearchProductPage.search_button, productData[1].name + date_time));
    test('should go to the product page', () => client.waitForExistAndClick(SearchProductPage.search_product_name.replace("%PRODUCT", productData[1].name + date_time)));
  ...
  scenario('Go back to the Back Office and check the created customer', () => {
    ...
    test('should check the email existence in the "Customers list"', async () => {
      ...
      await client.search(Customer.customer_filter_by_email_input, data.customer.email.replace("%ID", date_time));
      await client.checkExistence(Customer.email_address_value, data.customer.email.replace("%ID", date_time), 6);
    });
  ...
  scenario('Go back to the Back Office and check the order details', () => {
    test('should go back to the Back Office', () => client.switchWindow(0));
  ...
  stock_common_scenarios.checkStockProduct(client, productData[0].name + date_time, Menu, Stock, "50", "300", "350");
  stock_common_scenarios.checkStockProduct(client, productData[1].name + date_time, Menu, Stock, "50", "300", "350");
  ...
  stock_common_scenarios.checkStockProduct(client, productData[0].name + date_time, Menu, Stock, "50", "0", "50");
  stock_common_scenarios.checkStockProduct(client, productData[1].name + date_time, Menu, Stock, "50", "0", "50");
  scenario('Check the movement of the "' + productData[0].name + date_time + '"', client => {
    stock_common_scenarios.checkMovementHistory(client, Menu, Movement, 2, "300", "-", "Customer Order", productData[0].reference, dateSyst, "John DOE", productData[0].name + date_time);
  }, 'stocks');
  scenario('Check the movement of the "' + productData[1].name + date_time + '"', client => {
    stock_common_scenarios.checkMovementHistory(client, Menu, Movement, 1, "300", "-", "Customer Order", productData[1].reference, dateSyst, "John DOE", productData[1].name + date_time);
  }, 'stocks');
