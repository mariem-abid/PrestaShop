...
scenario('Shopping carts view', () => {
  ...
  scenario('View the shopping carts in the Back Office', client => {
  ...
  scenario('Create order in the Front Office', client => {
    ...
    scenario('Choose "SHIPPING METHOD"', client => {
      // test('should choose shipping method "PrestaShop"', () => client.waitForExistAndClick(CheckoutOrderPage.prestashop_shipping));
  ...
  scenario('Check order in the Back Office', client => {
    ...
    scenario('Check order informations', client => {
      ...
      test('should check the first product base price', () => client.checkTextValue(OrderPage.product_basic_price.replace('%NUMBER', 1), global.tab["unit_price_first_product"]));
      test('should check the second product base price', () => client.checkTextValue(OrderPage.product_basic_price.replace('%NUMBER', 2), global.tab["unit_price_second_product"]));
